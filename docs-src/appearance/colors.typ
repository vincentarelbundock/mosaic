...
The deck lives in one #repo-file("docs-src/examples/embedded/appearance/_palette-deck.typ")[shared file] exporting a single `deck` function, exactly like the running example on the #link("themes.html")[Themes] page. Each wrapper imports it, names one bundled palette, and renders:
