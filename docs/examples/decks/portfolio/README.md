# Greyscale: a vendored Mosaic theme

This example packages its monochrome visual language as a complete, callable
Mosaic facade in `theme.typ`: `setup`, `layouts`, and the ordinary public Mosaic
commands are imported together as `m`. All photographs are stored as true
greyscale images, so the black, white, and neutral-gray palette is preserved in
every output format.

The 14-slide demonstration adapts the *Photojournalist Portfolio* presentation
