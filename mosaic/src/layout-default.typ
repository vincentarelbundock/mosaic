...
#import "layout-support.typ": region-surface, track-children
...
#let validate-fields(fields) = {
  ...
  if type(inverted) != array or inverted.any(
    ...
    fail(
      "layout \"default\" inverted must be an array containing "
  ...
  for name in ("fill", "background", "text", "align", "inset") {
    ...
    if type(value) != dictionary {
      fail("layout \"default\" " + name + " must be a dictionary")
    }
    let unknown = value.keys().filter(key => key not in regions)
    if unknown.len() > 0 {
      fail(
        "layout \"default\" " + name + " has unknown "
  ...
  for (region, value) in fields.text {
    if type(value) != dictionary {
      fail(
        "layout \"default\" text style for cell "
  ...
  for (region, value) in fields.background {
    if value != none and type(value) != content {
      fail(
        "layout \"default\" background for cell "
...
/// layout resolves to a vertical Mosaic split whose body child is a
