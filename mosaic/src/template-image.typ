// Construction, validation, and resolution of the image template.
...
#let validate-fields(fields) = {
  ...
  if type(variant) != str or variant not in variants {
    fail(
      "template \"image\" has unsupported variant " + repr(variant)
  ...
  if tracks != auto and (
    ...
    fail(
      "template \"image\" tracks must be auto "
        + "or an array of native Typst track sizes",
    )
  }
  if variant in directional-variants and tracks != auto and tracks.len() != 2 {
    fail("template \"image\" split variants require exactly two tracks")
  }
  if variant not in directional-variants and tracks != auto {
    fail("template \"image\" tracks apply only to split variants")
  }
  let path = fields.path
  let alt = fields.alt
  let fit = fields.fit
  if path != none and not valid-path(path) {
    fail("template \"image\" path must be a non-empty string, native path, or none")
  }
  if alt != none and type(alt) != str {
    fail("template \"image\" alt must be a string or none")
  }
  if type(fit) != str or fit not in ("cover", "contain", "stretch") {
    fail(
      "template \"image\" fit must be \"cover\", \"contain\", or \"stretch\"",
    )
  }
  if path == none and (alt != none or fit != "cover") {
    fail("template \"image\" alt and fit require path")
  }
  fields
}
