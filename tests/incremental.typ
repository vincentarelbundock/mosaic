...
#mosaic.slide[
  #mosaic.on(1, after: "visible")[Always after step 1.]
  #mosaic.on("2-3", before: "dimmed", after: "removed")[Middle.]
  #mosaic.on("3-")[Last.]
]

// Two pages: the active cell changes and removed tracks reflow.
#let changing-columns = mosaic.grid.h(
  mosaic.on(1, after: "removed", mosaic.grid.cell(id: "first")),
  mosaic.on("2-", before: "removed", mosaic.grid.cell(id: "second")),
)
...
#mosaic.slide[
  #mosaic.replace([short], [a longer alternative])
]
...
#mosaic.slide[
  ...
  $
  $
    f(x) = #mosaic.replace([$x^2 + 2x + 1$], [$(x + 1)^2$])
  $
]
...
#let command-canvas = mosaic.reduce.with(
...
#let fletcher-diagram = mosaic.reduce.with(
