...
#assert("default" in mosaic.layouts)

#let default-grid = mosaic.layouts.default()
#let resolved-default = resolve-layout(default-grid, settings)
#assert(resolved-default.tracks == (auto, 1fr, auto))
#assert(grid-test.count(resolved-default) == 3)

#let basic-grid = mosaic.layouts.default(variant: "body")
...
#let structured-grid = mosaic.layouts.default(
  columns: 2,
  tracks: (2fr, 1fr),
)
...
#let header-body = mosaic.slide(grid: header-body-layout)[Header][Body]

#let body-footer-layout = mosaic.layouts.default(variant: "body-footer")
...
#let body-footer = mosaic.slide(grid: body-footer-layout)[Body][Footer]
